  ...
  local res = ""
	if (config.bufferline_integration == false) then
		for i, v in ipairs(_G.cp_problems) do
		  if vim.api.nvim_get_current_tabpage() == v.tab_id then res = res .. "%#CpfNA#"
...
function M:wincmd(type, cmd)
  ...
  if index == 0 then return end
	vim.api.nvim_win_call(index, function() vim.cmd(cmd) end)
end
...
function M:test(t)
  if not self.result[t] then return end
